...
public class MainWindow extends javax.swing.JFrame {
	...
	public final void loadCategories() {
		try {
			ArrayList<Category> categories = RecipeDB.getInstance()
					.getCategories();
	...
	public void loadRecipe(Recipe recipe) {
		try {
			recipe.Ingredients = RecipeDB.getInstance().getIngredients(
					recipe.ID);
	...
	private void initComponents() {
		...
		stateControl = new SpeechStateControl(new Dimension(this.getWidth(),
				170));
		stateControl.setLocation(0, this.getHeight() - 170);
		this.add(stateControl);

		back = new ImagePanel("images/background.png", new Dimension(
				this.getWidth() - 5, this.getHeight() - 100));
		this.add(back);
	}
	...
	private void jList1ValueChanged(javax.swing.event.ListSelectionEvent evt) {
		if (!recipeMode) {
			...
			if (c != null) {
				try {
					ArrayList<Recipe> recipes = RecipeDB.getInstance()
							.getRecipes(c.ID);
