...
public abstract class AbstractResultHandler extends Thread {
	...
	/**
	 * Registers a command that a grammar rule can trigger by its name. Command
	 * names map uniquely to SpeechCommandHandler objects that perform the
	 * command.
	 * 
	 * @param commandName
	 * @param command
	 */
	protected void registerCommand(String commandName,
			ISpeechCommandHandler command) {
	 ...
	 *            the rule to add
	 * @param command
	 *            the registered command this rule triggers
	 * @param commandArg
	 *            an argument to be passed to that command when the rule is
	 *            triggered
	 * @throws GrammarException
	 * @throws IOException
	 * @throws JSGFGrammarParseException
	 * @throws JSGFGrammarException
	 */
	protected void addGrammarRule(String grammarRule, String command,
			String commandArg) throws GrammarException, IOException,
	...
	private class SpeechThread implements Runnable {
		...
		public SpeechThread(String tx) {
			text = tx;
		}

		public void run() {
			RATextToSpeech.speak(text);
		}
	}
	...
	private class DoHandle implements Runnable {
		...
		public DoHandle(ISpeechEventListener listener, RecognizerState state,
				String arg) {
	...
	private synchronized void waitTillReady() {
		/*
		 * if (welcomeThread != null && welcomeThread.isAlive()) { try {
		 * welcomeThread.join(); } catch (InterruptedException e) {
	...
	public void run() {
		...
		while (bRunning) {
			Result result = RASpeechRecognizer.getInstance().getRecognizer()
					.recognize();
			if (result != null) {
				...
				if (!bestResult.equals("")) {
					...
					try {
						RuleGrammar ruleGrammar = RASpeechRecognizer
								.getInstance().getRuleGrammar();
						RuleParse ruleParse = ruleGrammar.parse(bestResult,
								null);
						if (ruleParse != null) {
							org.mozilla.javascript.Context.enter(); // so no
																	// threading
																	// issue
																	// arrises
																	// on
																	// getTagsParser
							RASpeechRecognizer.getTagsParser().parseTags(
									ruleParse);
							String command = (String) RASpeechRecognizer
									.getTagsParser().get("command");
							String arg = (String) RASpeechRecognizer
									.getTagsParser().get("arg");
						...
						System.out.println("Result action failed:"
								+ ex.getMessage());
					}
				}
			}
